...
// Fetch FX data for current and 6 months history & predict
async function getFXData(base, target, months) {
  ...
  // Get historical daily FX rates
  let historyUrl = `https://api.exchangerate.host/timeseries?start_date=${dateStr(past)}&end_date=${dateStr(today)}&base=${base}&symbols=${target}`;
  let histData = await safeFetch(historyUrl);

  // Get current
  let currUrl = `https://api.exchangerate.host/latest?base=${base}&symbols=${target}`;
  let currData = await safeFetch(currUrl);

  // Extract rates
  let rates = [];
  for (let d in histData.rates) {
    rates.push({ date: d, rate: histData.rates[d][target] });
  }
  rates.sort((a, b) => new Date(a.date) - new Date(b.date));
  // Downsample to 1 per week for regression
  let weekly = [];
  for (let i = 0; i < rates.length; i += 7) weekly.push(rates[i].rate);

  // Linear regression for prediction
  let x = weekly.map((_, i) => i);
  let y = weekly;
  let lr = linearRegression(x, y);
  // Predict n months ahead (linear extrapolation)
  let pred = lr.a * (x.length + Math.floor(months / 4)) + lr.b;
  let confidence = 1 - lr.residual / (Math.abs(lr.b) + 1e-8);
  // Build fxCurve for each month
  let fxCurve = [];
  for (let i = 1; i <= months; ++i) {
    let weekIdx = x.length + Math.floor(i / 4);
    let predFx = lr.a * weekIdx + lr.b;
    fxCurve.push(predFx);
  }
...
// Simple linear regression: y = a*x + b; returns {a, b, residual}
function linearRegression(x, y) {
  ...
  let a = (n * sumxy - sumx * sumy) / (n * sumxx - sumx * sumx + 1e-10);
  let b = (sumy - a * sumx) / n;
  let residual = x.reduce((acc, val, i) => acc + Math.abs(y[i] - (a * val + b)), 0) / n;
  return { a, b, residual };
}
...
function suggestionHTML({ goal, months, rate, budget, inputCurrency, monthlyCurrency, goalCurrency, monthly, fxData, finalValueGoal, totalInvestedMonthly }) {
  ...
  for (let cur of CURRENCIES) {
    ...
    let otherFxData = fxData.monthlyToGoal;
    if (cur.code !== goalCurrency) {
      // Need to fetch or estimate
      // We'll use current rates for this quick suggestion
      let fxRate = 1;
      if (goalCurrency !== cur.code) {
        fxRate = fxData.monthlyToGoal.current; // fallback
      }
